...
require_once "ValuatumSoapProtocol.php";
...
class ValuatumQueryBuilder
{
     ...
     * @return none
     */
	function __construct( $soapUsername = "developer", $soapPassword = "L1bmXW1iUZkJ" )
	{
    ...
    public function update($table, $alias = "")
	{
		...
		if (!$this->validateTableName($table)) {
			$this->lastErrorCode = $valuatumQBErrorList[0][0];
			$this->lastErrorMsg  = $valuatumQBErrorList[0][1] . ": " . $table;
			return $this;
		}
	...
	protected function validateSetFieldName($field)
	{
		...
		for ($i = 0; $i < $no; $i++) {
			...
			if ($this->updateTableList[$i]["alias"] != "") {
				...
				if (strlen($testField) > $len && 0 == strcmp(substr($testField, 0, $len), $this->updateTableList[$i]["alias"] ) && $testField[$len] == '.') {
	...
	protected function sendSoapRequest($request)
	{
		try{
			...
			$result = $client->__soapCall(
										$request["soapAction"], $request["paramList"], null, new SoapHeader(
